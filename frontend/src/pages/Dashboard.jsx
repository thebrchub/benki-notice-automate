...
import { Activity, FileText, ArrowRight, Search, Bell, Clock, AlertCircle, ChevronRight } from 'lucide-react';
...
const Dashboard = () => {
  ...
  // DUMMY DATA
  const recentCases = Array.from({ length: 5 }, (_, i) => ({
    id: `CASE-${100 + i}`,
    title: 'Tech Corp Ltd vs ACIT Circle 4(1)',
    date: '2 hrs ago',
  ...
  return (
    <div className="flex min-h-screen bg-gray-100 dark:bg-[#000000] font-sans transition-colors duration-300">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Header - SMART SCROLL ANIMATION APPLIED HERE */}
        <header 
        ...
        >
          <div className="flex flex-col">
            <h2 className="text-xl font-bold text-zinc-900 dark:text-white">
              {activeTab === 'overview' ? 'Overview' : activeTab === 'portal' ? 'ITAT Portal' : 'Analysis'}
            </h2>
          ...
          <div className="flex items-center gap-4">
             ...
             <div className="hidden md:flex items-center bg-white dark:bg-[#18181b] border border-zinc-200 dark:border-zinc-800 rounded-full px-4 py-2 w-64 shadow-sm dark:shadow-none focus-within:ring-2 ring-zinc-500/20 transition-all">
                ...
                <input 
                    ...
                    className="bg-transparent border-none focus:outline-none ml-2 text-sm text-zinc-900 dark:text-zinc-200 w-full placeholder-zinc-400"
                />
            </div>
        ...
        <main className="flex-1 p-8 pt-4">
          ...
          {activeTab === 'overview' && (
            <div className="flex flex-col gap-6 animate-in fade-in duration-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="bg-zinc-900 dark:bg-white rounded-2xl p-6 flex flex-col justify-between text-white dark:text-black shadow-lg">
                   ...
                   <button 
                      ...
                      className="mt-4 w-full py-3 bg-white dark:bg-black text-black dark:text-white font-bold rounded-xl hover:opacity-90 transition-opacity flex items-center justify-center gap-2 text-sm"
                   >
              ...
              {/* Layout Split: Main Content (2/3) + Sidebar (1/3) */}
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
                ...
                <Card className="lg:col-span-2 overflow-hidden flex flex-col">
                  <div className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center bg-zinc-50/50 dark:bg-zinc-900/50">
                    ...
                    <button onClick={() => setActiveTab('analysis')} className="text-xs font-medium text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition-colors">
                      View All
                    </button>
                  </div>
                    ...
                    {recentCases.map((item, i) => (
                      <div key={i} className="px-6 py-4 flex items-center justify-between hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors group cursor-pointer">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-200">{item.title}</p>
                            <p className="text-xs text-zinc-500 dark:text-zinc-500 mt-0.5">{item.id}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-4">
                   ...
                   <Card className="p-6">
                      ...
                      <div className="space-y-4">
                          ...
                          <div className="flex justify-between items-center text-sm">
                             ...
                             <span className="font-mono text-zinc-900 dark:text-white">0</span>
                          </div>
                          <div className="pt-4 border-t border-zinc-100 dark:border-zinc-800">
                             <div className="flex justify-between mb-2">
                                <span className="text-xs font-semibold text-zinc-700 dark:text-zinc-300">Processing Node</span>
                                <span className="text-xs text-zinc-400">Idle</span>
                             </div>
                             <div className="w-full h-1.5 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                                <div className="h-full bg-zinc-900 dark:bg-zinc-500 w-[5%]"></div>
                             </div>
                          </div>
                      </div>
                   </Card>
                   ...
                   <div className="bg-gradient-to-br from-zinc-900 to-black dark:from-zinc-800 dark:to-zinc-900 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden border border-zinc-800">
                      <div className="relative z-10">
                        ...
                        <p className="text-zinc-400 text-sm mt-2 leading-relaxed">
                            Use the 'Batch Export' feature in the Analysis tab to download 50+ summaries at once.
                        </p>
                      </div>
                      <div className="absolute -bottom-4 -right-4 text-zinc-800 dark:text-zinc-700 opacity-20">
                         <Activity size={120} />
                      </div>
                   </div>
                </div>
          ...
          {/* --- VIEW 2: PORTAL --- */}
          <div className={`${activeTab === 'portal' ? 'block' : 'hidden'} h-[calc(100vh-8rem)] w-full`}>
             <Card className="h-full w-full overflow-hidden p-0 border-0 shadow-none">
                <ITATViewer />
             </Card>
          </div>
          ...
          <div className={`${activeTab === 'analysis' ? 'block' : 'hidden'}`}>
             {isProcessing && (
                <Card className="mb-6 p-6 border-l-4 border-l-blue-500">
